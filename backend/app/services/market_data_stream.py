"""
PERFECTED: 5PAISA ONLY Market Data Stream
Restricts streaming to 5paisa accounts only.
"""
import asyncio
import json
import logging
...
class MarketDataStream:
    """Real-time market data streaming: 5PAISA ONLY"""
    ...
    async def start_streaming(self):
        """Start streaming market data for 5PAISA accounts ONLY"""
        self.running = True
        all_accounts = await AccountService.get_all_accounts(self.db, enabled_only=True)
        # Filter for 5paisa only
        accounts = [acc for acc in all_accounts if acc.broker_name == BrokerName.FIVEPAISA.value]
        ...
        for account in accounts:
            logger.info(f"Starting 5paisa stream for {account.nickname}")
            asyncio.create_task(self._stream_fivepaisa(account))

    async def _stream_fivepaisa(self, account: Account):
        """5paisa Stream with explicit subscription."""
        try:
            # The corrected structure for 5paisa subscription
            subscription_payload = {
                "Method": "Conf",
                "Operation": "Subscribe",
                "ClientCode": account.trading_login_id,
                "InstrumentList": [
                    {"Exchange": "N", "ExchangeType": "C", "ScripCode": 25},      # NIFTY
                    {"Exchange": "N", "ExchangeType": "C", "ScripCode": 26},      # BANKNIFTY
                    {"Exchange": "N", "ExchangeType": "C", "ScripCode": 1594},    # RELIANCE
                    # Add more scrip codes here or implement dynamic subscription
                ]
            }

            # 5paisa WebSocket Feed URL
            ws_url = "wss://openfeed.5paisa.com/feeds" 
            
            async with websockets.connect(ws_url) as websocket:
                # Send the subscription message
                await websocket.send(json.dumps(subscription_payload))
                logger.info(f"5paisa subscription sent for {account.trading_login_id}")
                ...
                while self.running:
                    ...
                    if scrip_code:
                        tick_data = {
                            ...
                            "change_percent": data.get("ChgPrc"),
                            "volume": data.get("Volume"),
                            "timestamp": datetime.now().isoformat(),
                        }
                        # Store in Redis using the specific key pattern
                        await set_cache(f"market:FIVEPAISA:{scrip_code}", tick_data, expire=10)

        except Exception as e:
            logger.error(f"5paisa stream error for {account.nickname}: {e}")
